...
- **Evidência:** validação determinística executada na worktree — os 3 JSON parseiam e declaram `temcomo`/`0.1.0`; o YAML parseia; **todos os caminhos declarados existem** (as 4 pastas de skill com seu `SKILL.md`, `./skills/`, `./agents/`, `./engine/temcomo.py`, e o `source: "./"`). Formatos espelhados dos exemplos reais instalados: `~/.claude