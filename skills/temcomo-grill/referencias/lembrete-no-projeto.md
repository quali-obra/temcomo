...
3. **Cite a evidência:** arquivo + `pergunta_id`. Parecer de drift sem essa citação é opinião.
4. **Divergência confirmada volta ao usuário.** Não edite o grill para caber no que foi feito nem reinterprete a resposta dele; se o usuário quiser mudar uma decisão, pare e reporte.
5. **Grill em silêncio não é autorização:** o que o grill não cobriu é pergunta para o usuário, não decisão de quem implementa.
~~~
