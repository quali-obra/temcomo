...
2. **Lançar o `entrevistador`** com o insumo (objetivo, brief de pesquisa, direção escolhida, respostas e anotações das rodadas anteriores, lacunas nomeadas pelo avaliador) e o **caminho absoluto da pasta da tarefa**. Ele produz `contratos/04-grill-rodada-N.json` (`grill-rodada-v1`) ali — handoff apontando arquivo fora da pasta da tarefa volta a ele. **Exceção:** subagente lançado em worktree isolado (`RUNBOOK.md` §4) não enxerga a pasta da tarefa que ainda não foi commitada: antes de lançar, copie a pasta da tarefa inteira para o mesmo caminho relativo dentro do worktree; ele entrega lá, e você traz de volta, byte a byte, só os arquivos que ele produziu, antes de qualquer gate — vale para tudo o que ele e o avaliador produzirem.
...
6. **Guardar e importar:** grave o que o usuário devolveu, byte a byte e sem sobrescrever, em `respostas/recebidas/grill-rodada-N.json` da pasta da tarefa (a subpasta não vem pronta: crie na primeira vez; reenvio: `grill-rodada-N-reenvio-2.json`) — **nunca em pasta temporária** — e importe **o arquivo que você acabou de gravar**: `python3 <raiz-do-plugin>/engine/temcomo.py importar-resposta .temcomo/tarefas/<tarefa>/respostas/recebidas/<arquivo-gravado>.json --tarefa .temcomo/tarefas/<tarefa>`. Recusa do motor (rodada errada, pergunta faltando, estado `pendente`) é para ser lida ao usuário e resolvida na origem, nunca contornada editando o JSON dele. A confirmação de que a rodada foi registrada é a própria saída do comando — `Resposta guardada em '…/respostas/grill-rodada-N.json'` — e a existência desse arquivo; o `status` mostra a **etapa** e o próximo gate, não o estado das rodadas.
...
8. **Pedir o consolidado candidato** ao `entrevistador` em `contratos/04-grill-consolidado-candidato-rodada-N.json` (decisões fechadas até aqui, cada uma citando `pergunta_id` + estado + escolha de origem).
...
- `contratos/04-grill-rodada-N.json`, `html/NN-grill-rodada-N.html`, `respostas/recebidas/grill-rodada-N.json` e `respostas/grill-rodada-N.json` por rodada, todos validados/importados pelo motor.
- `contratos/04-grill-consolidado-candidato-rodada-N.json` de cada rodada avaliada, `contratos/04-grill-consolidado.json` validado + documentos de contexto em `pesquisas/` ou `contratos/`.
- **Tudo na pasta da tarefa, dentro do projeto** — é a referência que orquestradores, juízes e revisores das etapas seguintes consultam quando suspeitam de drift.
...
- **Não grave nem aceite artefato do grill fora da pasta da tarefa** (`/tmp`, scratchpad, `Downloads`): o que fica lá some com a sessão e deixa de ser referência. Entrega em worktree isolado só conta depois que você a copia para a pasta da tarefa.
