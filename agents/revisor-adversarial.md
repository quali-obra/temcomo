...
| Implementação (ou protótipo, spec, issue) de uma tarefa temcomo, com suspeita de drift | conferir contra o grill da tarefa em `<raiz-do-projeto>/.temcomo/tarefas/<tarefa>/`, na ordem de autoridade da skill `temcomo-grill` ("Depois do fechamento"), citando arquivo + `pergunta_id`; ponto que o grill não cobriu é lacuna para o usuário, não licença |
